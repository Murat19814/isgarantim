...
import { createOffer, updateOffer, OfferError } from "@/lib/services/offers";

/** POST — hizmet veren bu talebe teklif verir (1. yıl ücretsiz). */
...
/** PATCH — hizmet veren kendi teklifini günceller. */
export async function PATCH(
  ...
  if (!hasRole(user, UserRole.PROVIDER))
    return NextResponse.json(
      { error: "Teklif güncellemek için 'Hizmet Veren' rolü gerekli." },
      { status: 403 },
    );
  ...
  try {
    const offer = await updateOffer(user.id, params.id, parsed.data);
